...
const GovernmentApprovalsPage = () => {
  ...
  const titleParts = governmentApprovalsData.bannerTitle
    ? governmentApprovalsData.bannerTitle.trim().split(/\s+/)
    : [];
  const highlightedTitle = titleParts[0] || "";
  const plainTitle = titleParts.slice(1).join(" ") || "";

  const imageUrl =
    "/images/governmentApprovals/government-approvals-banner.png";
